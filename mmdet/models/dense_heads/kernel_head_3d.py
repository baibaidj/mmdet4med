...
class ConvKernelHead3D(BaseModule):
    ...
    def __init__(self,
                 ...
                 init_cfg = None, 
                 **kwargs):
        super(ConvKernelHead3D, self).__init__(init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        if self.semantic_fpn:
            if self.loss_seg.use_sigmoid:
                self.conv_seg = nn.Conv3d(self.out_channels, self.num_classes,
                                          1)
            else:
                self.conv_seg = nn.Conv3d(self.out_channels,
