...
class DyConv(nn.Module):
    ...
    def forward(self, x):
        ...
        for level, name in enumerate(feature_names): # 1/4, 1/8, 1/16, 1/32

            feature = x[name]

            offset_mask = self.offset(feature)
            offset = offset_mask[:, :18, :, :] # B,18,H,W()
            mask = offset_mask[:, 18:, :, :].sigmoid() # B,9,H,W (0~1)
            conv_args = dict(offset=offset, mask=mask)

            temp_fea = [self.DyConv[1](feature, **conv_args)] # stride = 1
            if level > 0:
                temp_fea.append(self.DyConv[2](x[feature_names[level - 1]], **conv_args)) # stride = 2, downsample
            if level < len(x) - 1:
                # # stride = 1 
