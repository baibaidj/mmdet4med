...
from ..utils.transformer3d import PatchEmbed3DNN, PatchMergingNN
...
class WindowMSA3d(BaseModule):
    ...
    def __init__(self,
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * window_size[0] - 1) * 
                        (2 * window_size[1] - 1) * 
                        (2 * window_size[2] - 1),
                        num_heads))  # 2*Wh-1 * 2*Ww-1 * 2*Ww-1, nH
        ...
        rel_position_index = self.triple_step_seq(self.window_size)
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = relative_position_bias.permute(
            2, 0, 1).contiguous()  # nH, Wh*Ww*Wd, Wh*Ww*Wd
    ...
    @staticmethod
    def triple_step_seq(window_size):
...
class ShiftWindowMSA3d(BaseModule):
    ...
    def forward(self, query, hwd_shape):
        B, L, C = query.shape
        H, W, D = hwd_shape
        assert L == H * W * D, 'input feature has wrong size'
        query = query.view(B, H, W, D, C)

        # pad feature maps to multiples of window size
        winpad4dim = lambda S: (self.window_size - S % self.window_size) % self.window_size
        pad_b = winpad4dim(H)
        pad_r = winpad4dim(W)
        pad_p = winpad4dim(D)
        # l: left, r: right, t: top, b: bottom, a: anteior, p: posterior
        # starting from the last dimension and moving forward.
        query = F.pad(query, (0, 0, 0, pad_p, 0, pad_r, 0, pad_b)) 
        H_pad, W_pad, D_pad = query.shape[1], query.shape[2], query.shape[3]
        ...
        if self.shift_size > 0:
            ...
            img_mask = torch.zeros((1, H_pad, W_pad, D_pad, 1), device=query.device)
            ...
            mask_windows = mask_windows.view( ## nW, window_size^3
                -1, self.window_size * self.window_size * self.window_size)
            attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
            # # nW, 1, window_size^3; nW, window_size^3, 1
        ...
        # B H' W' C
        shifted_x = self.window_reverse(attn_windows, H_pad, W_pad, D_pad)
    ...
    def window_reverse(self, windows, H, W, D):
        """
        Args:
...
class SwinBlock3d(BaseModule):
    ...
    def __init__(self,
        ...
        super(SwinBlock3d, self).__init__()
...
class SwinTransformer3D(BaseModule):
    ...
    def __init__(self,
        ...
        self.drop_after_pos = nn.Dropout3d(p=drop_rate)
    ...
    def init_weights(self, pretrained = None):
        ...
        if self.init_cfg is None:
            ...
            if state_dict.get('absolute_pos_embed') is not None:
                ...
                N2, C2, *spatial_shape= self.absolute_pos_embed.size()
                if len(spatial_shape) == 3: is_3d, spatial_dim = True, 3
                else: is_3d, spatial_dim = False, 2

                if N1 != N2 or C1 != C2 or L != np.prod(spatial_shape):
                    logger.warning('Error in loading absolute_pos_embed, pass')
                else:
                    permute_order = (0, 4, 1, 2, 3) if is_3d else (0, 3, 1, 2)
                    state_dict['absolute_pos_embed'] = absolute_pos_embed.view(
                        N2, *spatial_shape, C2).permute(*permute_order).contiguous()
            ...
            for table_key in relative_position_bias_table_keys:
                ...
                if nH1 != nH2:
                    ...
                    table_pretrained_resized = F.interpolate(
                        table_pretrained.permute(1, 0).reshape(1, nH1, *[S1]*spatial_dim),
                        size=[S2]*spatial_dim,
                        mode='bicubic')
    ...
    def forward(self, x):
        ...
        # print(f'Embed {hw_shape}', x.shape)
        if self.use_abs_pos_embed:
