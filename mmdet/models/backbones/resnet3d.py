...
from ..utils.ccnet_pure import CCAttention3D
...
class BasicBlock3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.planes = planes
        self.spatial_stride = stride if stride is not None else spatial_stride
        self.temporal_stride = stride if stride is not None else temporal_stride
        ...
        self.conv1_stride_s = self.spatial_stride
        self.conv2_stride_s = 1
        self.conv1_stride_t = self.temporal_stride
        ...
        if self.non_local:
            self.non_local_block = CCAttention3D(self.conv2.norm.num_features, **self.non_local_cfg)
            #NonLocal3d(self.conv2.norm.num_features, **self.non_local_cfg)
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.spatial_stride = stride if stride is not None else spatial_stride 
        self.temporal_stride = stride if stride is not None else temporal_stride 
        self.dilation = dilation = _triple(dilation)
        self.style = style
        self.inflate = inflate
        ...
        if self.style == 'pytorch':
            ...
            self.conv2_stride_s = self.spatial_stride
            self.conv1_stride_t = 1
            self.conv2_stride_t = self.temporal_stride
        else:
            self.conv1_stride_s = self.spatial_stride
            self.conv2_stride_s = 1
            self.conv1_stride_t = self.temporal_stride
        ...
        if self.inflate:
            if inflate_style == '3x1x1':
                ...
                conv1_padding = (1, 0, 0)
                conv2_kernel_size = (1, 3, 3)
                conv2_padding = (0, dilation[1], dilation[2])
            else:
                conv1_kernel_size = (1, 1, 1)
                conv1_padding = (0, 0, 0)
                conv2_kernel_size = (3, 3, 3)
                conv2_padding = (1, dilation[1], dilation[2])
        else:
            conv1_kernel_size = (1, 1, 1)
            conv1_padding = (0, 0, 0)
            conv2_kernel_size = (1, 3, 3)
        ...
        if self.non_local:
            ...
            #                                   **self.non_local_cfg)
            self.non_local_block = CCAttention3D(self.conv3.norm.num_features, **self.non_local_cfg)
...
class ResNet3d(nn.Module):
    ...
    def _inflate_weights(self, logger):
        ...
        state_dict_r2d = _load_checkpoint(self.pretrained, torch.device('cpu'))
        if 'state_dict' in state_dict_r2d:
            state_dict_r2d = state_dict_r2d['state_dict']
            
        state_dict_temp = {k.replace('backbone.', '') : v for k, v in state_dict_r2d.items() if 'backbone' in k}
        if len(state_dict_temp) > 0: state_dict_r2d = state_dict_temp

        inflated_param_names = []
        for name, module in self.named_modules():
            # if 'stage4' in name : print(f'{name}', module)
            if isinstance(module, ConvModule):
                ...
                if name + '.weight' in state_dict_r2d:
                    # print(name, module)
                    shape_model_weight = module.weight.data.shape
                    shape_state_weight = state_dict_r2d[name + '.weight'].shape
                    # if shape_state_weight[2:] == (1, 1): continue
                    if len(shape_model_weight) == 5:
                        if shape_state_weight != shape_model_weight[:2] + shape_model_weight[3:]:
                                logger.warning(f'Weight shape mismatch for '
                                            f': {name} : '
                                            f'3d weight shape: {shape_model_weight}; '
                                            f'2d weight shape: {shape_state_weight}. '
                                            f'module detail {module}')
                        else:
                            self._inflate_conv_params(module, state_dict_r2d,
                                                        name,
                                                        inflated_param_names)
                    elif len(shape_model_weight) == 1 :
    ...
    def _make_stem_layer(self, in_channels, stem_channels, stem_stride_1, stem_stride_2):
