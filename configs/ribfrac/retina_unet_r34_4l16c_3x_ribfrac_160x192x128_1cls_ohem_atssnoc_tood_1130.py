...
model = dict(
        bbox_head = dict(stacked_convs=6, verbose = False, 
                        initial_loss_cls=dict(loss_weight=4.0), 
        ...
        train_cfg=dict(initial_epoch=2, alpha=1, beta=6,
            initial_assigner = dict(topk = 4, center_within = True), 
            assigner = dict(topk = 4, verbose = False), 
            sampler=dict(num = 32, pool_size = 20, pos_fraction=0.4),
        test_cfg = dict(nms_pre=400, score_thr=0.1, max_per_img=64, sw_batch_size = 8, overlap=0.4),
        )                
    )
...
# CUDA_VISIBLE_DEVICES=4 python tools/train.py configs/ribfrac/retina_unet_r34_4l16c_3x_ribfrac_160x192x128_1cls_ohem_atssnoc_tood_1130.py 
