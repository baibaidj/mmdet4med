...
data = dict(samples_per_gpu = 2, workers_per_gpu= 6, 
            train=dict(sample_rate = 0.5, json_filename = 'dataset_1130_truesplit.json', img_dir=img_dir, key2suffix = key2suffix), 
            val=dict(sample_rate = 0.3, json_filename = 'dataset_1130_truesplit.json', img_dir=img_dir, key2suffix = key2suffix), 
...
model = dict(
        ...
        train_cfg= dict(
            initial_epoch=3, alpha=1, beta=6, pos_weight=1.0,
            initial_assigner = dict(topk = 6, center_within = False, verbose = False), 
            assigner = dict(topk = 6, verbose = False), 
            sampler=dict(num = 48, pool_size = 20, pos_fraction=0.4)
            ), 

        test_cfg = dict(nms_pre=400, score_thr=0.25, max_per_img=48, sw_batch_size = 8, overlap=0.4),                
    )
