...
class IO4Png():
    ...
    def write(self, img_2d, file_name, mask_2d = None, is_3_channel = False, verbose = False):
        """
        ...
        if verbose: print('Img extreme', img_min, img_max)
        ...
        if verbose: print('Store extreme', info_channel[1, :3])
...
def read_png2array(png_path, label_color='red'):  # , label_mode='fill'
    ...
    if label_values.size > 0:
        for k in label_values:
            # print(k, label_start + k)
            mask_k = np.array(mask_on_image_channel == label_start + k, dtype=np.uint8) * k
            mask_multi_label += mask_k
...
# def contour_mask3labels(mask_2d):
#     unique_labels = np.unique(mask_2d)[1:]
#     mask_new = np.zeros(mask_2d.shape, dtype=np.uint8)
#     for l in unique_labels:
#         contours_list = find_contours(np.array(mask_2d == l, dtype=np.uint8))
#         # contour_list的每个元素是一个n*3的矩阵，第一列是x/columns 列, 第二列是y/rows 行，第三列是z
#         for contour in contours_list:
#             contour = np.array(contour, dtype=np.int16)
#             mask_temp = np.zeros(mask_2d.shape, dtype=np.uint8)
#             # 矩阵的索引顺序是 先行后列
#             mask_temp[contour[:, 1], contour[:, 0]] = 1
#             mask_temp = np.array(binary_dilation(mask_temp, disk(1)), dtype=np.uint8) * l
#             mask_new = np.amax(np.stack([mask_new, mask_temp], axis=0), axis=0)
#
#     return mask_new
