...
from ..utilsmm import get_root_logger
...
class BasicBlock3d(nn.Module):
    ...
    def __init__(self,
        ...
        if self.inflate:
            conv1_kernel_size = (3, 3, 1 if self.reduce_conv3d4z else 3)
            conv1_padding = (dilation, dilation, 0 if self.reduce_conv3d4z else 1)
            conv2_kernel_size = (3, 3, 3)
            conv2_padding = (1, 1, 1)
        else:
            conv1_kernel_size = (3, 3, 1)
            conv1_padding = (dilation, dilation, 0)
            conv2_kernel_size = (3, 3, 1)
            conv2_padding = (1, 1, 0)
        ...
        self.conv2 = ConvModule(
            ...
            conv2_kernel_size,
            stride=(self.conv2_stride_s,
                    self.conv2_stride_s, self.conv2_stride_t),
    ...
    def __init__(self,
        ...
        if self.inflate:
            if inflate_style == '3x1x1':
                conv1_kernel_size = (1, 1, 3)
                conv1_padding = (0, 0, 1)
                conv2_kernel_size = (3, 3, 1)
                conv2_padding = (dilation, dilation, 0)
            else:
                conv1_kernel_size = (1, 1, 1)
                conv1_padding = (0, 0, 0)
                conv2_kernel_size = (3, 3, 3)
                conv2_padding = (dilation, dilation, 1)
        else:
            conv1_kernel_size = (1, 1, 1)
            conv1_padding = (0, 0, 0)
            conv2_kernel_size = ( 3, 3, 1)
        ...
        self.conv1 = ConvModule(
            ...
            conv1_kernel_size,
            stride=(self.conv1_stride_s,
                    self.conv1_stride_s, self.conv1_stride_t),
            padding=conv1_padding,
            bias=False,
            conv_cfg=self.conv_cfg,
            norm_cfg=self.norm_cfg,
