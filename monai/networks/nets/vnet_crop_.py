...
class VNet_crop_(nn.Module):
    """
    V-Net based on `Fully Convolutional Neural Networks for Volumetric Medical Image Segmentation, a dynamic channels set, and remove first skip connection in InputTransition
    ...
    def __init__(
        ...
        self.aux_sdm = aux_sdm
        n = init_channels
        self.in_tr = InputTransition(spatial_dims, in_channels, n, act)
        self.down_tr32 = DownTransition(spatial_dims, n, 1, act)
        self.down_tr64 = DownTransition(spatial_dims, n*2, 2, act)
        self.down_tr128 = DownTransition(spatial_dims, n*4, 3, act, dropout_prob=dropout_prob, dropout_dim=dropout_dim)
        self.down_tr256 = DownTransition(spatial_dims, n*8, 2, act, dropout_prob=dropout_prob, dropout_dim=dropout_dim)
        self.up_tr256 = UpTransition(spatial_dims, n*16, n*16, 2, act, dropout_prob=dropout_prob, dropout_dim=dropout_dim)
        self.up_tr128 = UpTransition(spatial_dims, n*16, n*8, 2, act, dropout_prob=dropout_prob, dropout_dim=dropout_dim)
        self.up_tr64 = UpTransition(spatial_dims, n*8, n*4, 1, act)
        self.up_tr32 = UpTransition(spatial_dims, n*4, n*2, 1, act)
        self.out_tr = OutputTransition(spatial_dims, n*2, out_channels, act)

        if self.aux_sdm: self.out_tr_sdm = OutputTransition(spatial_dims, n*2, 1, act)
    ...
    def forward(self, x):
        ...
        x_feat = self.up_tr32(x, out16)
        x = self.out_tr(x_feat)

        if self.training and self.aux_sdm:
            x_sdm = self.out_tr_sdm(x_feat)
            return x, x_sdm
