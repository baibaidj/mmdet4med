...
class ToTensord(MapTransform):
    ...
    def __init__(self, keys: KeysCollection, use_gpu = False, local_rank = None) -> None:
        """
        Args:
            keys: keys of the corresponding items to be transformed.
                See also: :py:class:`monai.transforms.compose.MapTransform`
        ...
        # print(f'2Tensor use gpu {self.use_gpu} local rank {self.local_rank}')
    ...
    def __call__(
        ...
        for key in self.keys:
            d[key] = self.converter(d[key])
            if self.use_gpu and (self.local_rank is not None):
                d[key] = d[key].to(torch.device(f"cuda:{self.local_rank}"))
        return d
...
class DataStatsd(MapTransform):
    ...
    def __call__(self, data: Mapping[Hashable, NdarrayTensor]) -> Dict[Hashable, NdarrayTensor]:
        d = dict(data)
        for idx, key in enumerate(self.keys):
            filename = d[f'{key}_meta_dict'].get('filename_or_obj', '')
            d[key] = self.printer(
                d[key],
                f'{key}_{filename}_{self.prefix[idx]}',
                self.data_shape[idx],
