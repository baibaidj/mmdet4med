...
class Rand3DElasticd(Randomizable, MapTransform):
    ...
    def __call__(
        ...
        if self.rand_3d_elastic.do_transform:
            ...
            with Timer(print_tmpl='\tElasticPrepare {:.3f} seconds'):
                self.rand_3d_elastic.device = device
                grid = torch.tensor(grid, device=device) #.to(device).astype(dtype)
                sigma = torch.tensor(self.rand_3d_elastic.sigma, device = device).float()
                gaussian = GaussianFilter(spatial_dims=3, sigma=sigma, truncated=3.0).to(device)
                ...
                self.rand_3d_elastic.rand_affine_grid.device = device
                grid = self.rand_3d_elastic.rand_affine_grid(grid=grid)

        for idx, key in enumerate(self.keys):
            self.rand_3d_elastic.resampler.device = d[key].device
